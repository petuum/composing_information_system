...
class TaggingTrainer(BaseTrainer):
    def __init__(self, config_data: Dict, config_model: Dict, device):
        super().__init__()
    ...
    def create_tp_config(self) -> Dict:
        tp_config: Dict = {
            ...
            "dataset": {"batch_size": self.config_data["train_batch_size"]},
    ...
    def build_model(self, num_classes):
        ...
        self.model = BERTClassifier(
            ...
            hparams={"num_classes": num_classes, "clas_strategy": "time_wise"},
        )
    ...
    def build_predictor(self, vocab_path):
        ...
        (
            self.config_model["tp_request"]["feature_scheme"]["output_tag"][
                "extractor"
            ]["vocab_path"]
        ) = vocab_path
    ...
    def build_val_pl(
        self, predictor, evaluator, evaluator_config, predictor_config
    ):
    ...
    def save_vocab(self, train_preprocessor):
        output_vocab_path = os.path.join(
            self.config_data["output_path"], "vocab.pkl"
        )
        with open(output_vocab_path, "wb") as vocab_file:
            pickle.dump(
                train_preprocessor.request["schemes"]["output_tag"][
                    "extractor"
                ].vocab,
                vocab_file,
            )
    ...
    def train(self):
        ...
        input_extractor = train_preprocessor.request["schemes"]["input_tag"][
            "extractor"
        ]
        output_extractor = train_preprocessor.request["schemes"]["output_tag"][
            "extractor"
        ]
        self.build_model(num_classes=len(output_extractor.vocab))
        lr_scheduler, optim = build_lr_decay_scheduler(
            self.model,
        ...
        val_pl = self.build_val_pl(
            predictor, evaluator, evaluator_config, predictor_config
        )
        # Start Training
        epoch: int = 0
        ...
        while epoch < self.config_data["num_epochs"]:
            ...
            train_err, train_total, train_sentence_len_sum = self.train_epoch(
                batch_iter, optim, lr_scheduler, input_extractor.get_pad_value()
            )
            logger.info(
                "%dth Epoch training, "
